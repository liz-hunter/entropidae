...
# === 🔧 USER-DEFINED PATHS ===
TAXDUMP_DIR = Path("/hpc/scratch/Elizabeth.Hunter/entropy/accession2taxid_masters/tax")  # contains nodes.dmp
SUMMARY_DIR = Path("/hpc/scratch/Elizabeth.Hunter/entropy/analysis/toxin_summaries")  # *_virid_summary.txt files
OUTPUT_DIR = Path("/hpc/scratch/Elizabeth.Hunter/entropy/analysis/entropy_final")  # where to write scored outputs
...
# === 📏 Compute distance between two taxids ===
...
def analyze_summary_file(file_path, parent_map):
    ...
    for idx, row in df.iterrows():
        ...
        for col in tax_columns:
            pred_tax = row[col]
            if pd.isna(pred_tax) or pred_tax == "0" or pred_tax == "":
